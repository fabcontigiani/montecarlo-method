...
for k in range(ITERACIONES):
    ...
    while jugador1.puntos < PUNTOS_PARA_GANAR and jugador2.puntos < PUNTOS_PARA_GANAR:
        ...
        # Jugador 1 Flor (Jugador 2 no puede cantar, solo responder)
        if (jugador1.mano[0].palo == jugador1.mano[1].palo == jugador1.mano[2].palo):
            if (jugador2.mano[0].palo == jugador2.mano[1].palo == jugador2.mano[2].palo):
                tantosJ1 = 20
                tantosJ2 = 20
                for carta in jugador1.mano:
                    if carta.numero not in (10,11,12):
                        tantosJ1 += carta.numero
                for carta in jugador2.mano:
                    if carta.numero not in (10,11,12):
                        tantosJ1 += carta.numero
                if tantosJ1 > tantosJ2:
                    jugador1.puntos += 6
                elif tantosJ2 > tantosJ1:
                    jugador2.puntos += 6
                else:
                    if esMano == 1:
                        jugador1.puntos += 6
